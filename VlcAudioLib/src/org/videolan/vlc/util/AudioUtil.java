...
public class AudioUtil {
	...
	public static void setRingtone(Media song, Context context) {
		...
		Uri uri = MediaStore.Audio.Media.getContentUriForPath(newringtone
				.getAbsolutePath());
		Uri newUri;
		try {
			context.getContentResolver().delete(
					...
					MediaStore.MediaColumns.DATA + "=\""
							+ newringtone.getAbsolutePath() + "\"", null);
	...
	public static void prepareCacheFolder(Context context) {
		if (LibVlcUtil.isFroyoOrLater() && AndroidDevices.hasExternalStorage()
				&& context.getExternalCacheDir() != null)
			CACHE_DIR = context.getExternalCacheDir().getPath();
		else
			CACHE_DIR = Environment.getExternalStorageDirectory().getPath()
	 ...
	 * @return
	 */
	private static String getCoverFromMediaStore(Context context,
			MusicInfo media) {
		ContentResolver contentResolver = context.getContentResolver();
		Uri uri = android.provider.MediaStore.Audio.Albums.EXTERNAL_CONTENT_URI;
		Cursor cursor = contentResolver.query(uri, new String[] {
	...
	private static String getCoverFromVlc(Context context, Media media)
		...
		if (artworkURL != null && artworkURL.startsWith("file://")) {
			...
			if (mArtist.length() == 0
					|| mAlbum.length() == 0
					|| mArtist.equals(context
							.getString(R.string.unknown_artist))
					|| mAlbum.equals(context.getString(R.string.unknown_album))) {
				/*
				 * If artist or album are missing, it was cached by title MD5
				 * hash
				 */
				MessageDigest md = MessageDigest.getInstance("MD5");
				byte[] binHash = md.digest((artworkURL + media.getTitle())
						.getBytes("UTF-8"));
				...
				artworkURL = ART_DIR + "/artistalbum/" + mArtist + "/" + mAlbum
						+ "/art.png";
			}
	 ...
	 */
	private static String getCoverFromVlc(Context context, MusicInfo media)
			throws NoSuchAlgorithmException, UnsupportedEncodingException {
		// String artworkURL = media.getArtworkURL();
		// if (artworkURL != null && artworkURL.startsWith("file://")) {
		// return Uri.decode(artworkURL).replace("file://", "");
	 ...
	 * @return
	 */
	private static String getCoverFromFolder(Context context, MusicInfo media) {
		File f = LibVlcUtil.URItoFile(media.getUrlPath());
		if (f != null && f.getParentFile() != null
				&& f.getParentFile().listFiles() != null)
			for (File s : f.getParentFile().listFiles()) {
				if (s.getAbsolutePath().endsWith("png")
	...
	@SuppressLint("NewApi")
	public synchronized static Bitmap getCover(Context context, Media media,
			int width) {
		...
		// if external storage is not available, skip covers to prevent slow
		// audio browsing
		...
		try {
			...
			cachePath = COVER_DIR + (hash >= 0 ? "" + hash : "m" + (-hash))
					+ "_" + width;
			...
			Log.i(TAG, "getCover-------->coverPath = " + coverPath
					+ " cachePath = " + cachePath);
	...
	public synchronized static Bitmap getCover(Context context,
			MusicInfo media, int width) {
		String coverPath = null;
		Bitmap cover = null;
		String cachePath = null;
		...
		try {
			...
			cachePath = COVER_DIR + (hash >= 0 ? "" + hash : "m" + (-hash))
					+ "_" + width;
			Log.i(TAG, "getCover-------->cachePath = " + cachePath);
			// try to get the cover from the LRUCache first
			BitmapCache cache = BitmapCache.getInstance(context);
			cover = cache.getBitmapFromMemCache(cachePath);
			if (cover != null)
	...
	private static Bitmap readCoverBitmap(Context context, String path,
		...
		int width = Util.convertDpToPx(dipWidth, context);
