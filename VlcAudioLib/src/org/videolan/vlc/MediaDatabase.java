...
public class MediaDatabase {
			...
			try {
				...
				try {
					db = SQLiteDatabase.openOrCreateDatabase(
							mContext.getDatabasePath(DB_NAME), null);
	...
	public synchronized Media getMedia(String location) {
		...
		if (cursor.moveToFirst()) {
			media = new Media(location, cursor.getLong(0),
					cursor.getLong(1),
					cursor.getInt(2),
	...
	public static void setPicture(Context context, Media m, Bitmap p) {
		...
		try {
			getInstance(context).updateMedia(m.getLocation(),
					mediaColumn.MEDIA_PICTURE, p);
