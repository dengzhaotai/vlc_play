...
public class MediaGroup extends Media {
	...
	public MediaGroup(Context context, Media media) {
		super(media.getLocation(), media.getTime(), media.getLength(),
				Media.TYPE_GROUP, BitmapUtil
						.getPictureFromCache(context, media), media.getTitle(),
				media.getArtist(), media.getGenre(), media.getAlbum(), media
						.getWidth(), media.getHeight(), media.getArtworkURL(),
				media.getAudioTrack(), media.getSpuTrack());
		mMedia = media;
	...
	public static List<MediaGroup> group(Context context, List<Media> mediaList) {
		ArrayList<MediaGroup> groups = new ArrayList<MediaGroup>();
		for (Media media : mediaList)
			insertInto(context, groups, media);
		return groups;
	}

	private static void insertInto(Context context,
			ArrayList<MediaGroup> groups, Media media) {
		...
		groups.add(new MediaGroup(context, media));
	}
}
