...
public class MediaLibrary {
	...
	public void loadMediaItems(Context context) {
		if (mLoadingThread == null
			...
			mLoadingThread = new Thread(new GetMediaItemsRunnable(context));
	...
	public synchronized static MediaLibrary getInstance(Context context) {
		if (mInstance == null)
			...
			if (item.getType() == Media.TYPE_AUDIO) {
				...
				case MODE_ARTIST:
					valid = name.equals(item.getArtist())
							&& (name2 == null || name2.equals(item.getAlbum()));
					break;
				case MODE_ALBUM:
	...
	private class GetMediaItemsRunnable implements Runnable {
		...
		public void run() {
			...
			try {
				libVlcInstance = VLCInstance.getLibVlcInstance(mContext);
			...
			if (mediaDirs.size() == 0) {
				...
				String storageDirs[] = AndroidDevices
						.getMediaDirectories(mContext);
			...
			try {
				...
				for (File file : mediaToScan) {
					...
					if (existingMedias.containsKey(fileURI)) {
						...
						MediaDatabase db = MediaDatabase.getInstance(mContext);
						db.addMedia(m);
						mItemListLock.writeLock().unlock();
					}
					if (isStopping) {
				...
				// hide progressbar in footer Edited by dzt 2014.08.08
	...
	private static class RestartHandler extends WeakHandler<MediaLibrary> {
		...
		public void handleMessage(Message msg) {
			...
			if (owner == null)
				return;
			owner.loadMediaItems(owner.mContext);
		}
	}
