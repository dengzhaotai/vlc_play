...
libvlc_media_t *new_media(jlong instance, JNIEnv *env, jobject thiz,
	...
	jboolean isCopy;
	const char *psz_location = (*env)->GetStringUTFChars(env, fileLocation,
			&isCopy);
...
static void vlc_event_callback(const libvlc_event_t *ev, void *data) {
	...
	jmethodID putInt = (*env)->GetMethodID(env, clsBundle, "putInt",
			"(Ljava/lang/String;I)V");
	jmethodID putFloat = (*env)->GetMethodID(env, clsBundle, "putFloat",
			"(Ljava/lang/String;F)V");
	jmethodID putString = (*env)->GetMethodID(env, clsBundle, "putString",
	...
	if (ev->type == libvlc_MediaPlayerPositionChanged) {
		...
		jstring sData = (*env)->NewStringUTF(env, "data");
		(*env)->CallVoidMethod(env, bundle, putInt, sData,
				ev->u.media_player_vout.new_count);
		(*env)->DeleteLocalRef(env, sData);
		...
		(*env)->CallVoidMethod(env, bundle, putString, item_uri,
				item_uri_value);
	...
	if (methodID) {
		(*env)->CallVoidMethod(env, eventHandlerInstance, methodID, ev->type,
				bundle);
	} else {
		LOGE("EventHandler: failed to get the callback method");
	}

	end: (*env)->DeleteLocalRef(env, bundle);
	if (isAttached)
...
void Java_org_videolan_libvlc_LibVLC_nativeInit(JNIEnv *env, jobject thiz) {
	...
	bool use_opensles = (*env)->CallIntMethod(env, thiz,
			methodId) == AOUT_OPENSLES;

	methodId = (*env)->GetMethodID(env, cls, "getVout", "()I");
	bool use_opengles2 = (*env)->CallIntMethod(env, thiz,
			methodId) == VOUT_OPENGLES2;
	...
	if (networkCaching > 0) {
		snprintf(networkCachingstr, sizeof(networkCachingstr),
				"--network-caching=%d", networkCaching);
		LOGD("Using network caching of %d ms", networkCaching);
	}

	methodId = (*env)->GetMethodID(env, cls, "getChroma",
			"()Ljava/lang/String;");
	...
	methodId = (*env)->GetMethodID(env, cls, "getSubtitlesEncoding",
			"()Ljava/lang/String;");
	jstring subsencoding = (*env)->CallObjectMethod(env, thiz, methodId);
	const char *subsencodingstr = (*env)->GetStringUTFChars(env, subsencoding,
			0);
	...
	methodId = (*env)->GetMethodID(env, cls, "getCachePath",
			"()Ljava/lang/String;");
	...
	enable_time_stretch ? "--audio-time-stretch" : "--no-audio-time-stretch",
			...
			"--avcodec-skiploopfilter", deblockstr, "--avcodec-skip-frame",
					enable_frame_skip ? "2" : "0", "--avcodec-skip-idct",
					enable_frame_skip ? "2" : "0",
			...
			"--androidsurface-chroma",
					chromastr != NULL && chromastr[0] != 0 ? chromastr : "RV32",
			/* XXX: we can't recover from direct rendering failure */
			(hardwareAcceleration == HW_ACCELERATION_FULL) ?
					"" : "--no-mediacodec-dr", };
	libvlc_instance_t *instance = libvlc_new(sizeof(argv) / sizeof(*argv),
			argv);
...
void Java_org_videolan_libvlc_LibVLC_setEventHandler(JNIEnv *env, jobject thiz,
...
void Java_org_videolan_libvlc_LibVLC_playMRL(JNIEnv *env, jobject thiz,
		jlong instance, jstring mrl, jobjectArray mediaOptions) {
	/* Release previous media player, if any */
	releaseMediaPlayer(env, thiz);

	/* Create a media player playing environment */
	libvlc_media_player_t *mp = libvlc_media_player_new(
			(libvlc_instance_t*) (intptr_t) instance);
	...
	static const libvlc_event_type_t mp_events[] = { libvlc_MediaPlayerPlaying,
			libvlc_MediaPlayerPaused, libvlc_MediaPlayerEndReached,
			libvlc_MediaPlayerStopped, libvlc_MediaPlayerVout,
	...
	libvlc_media_t* p_md = libvlc_media_new_location(
			(libvlc_instance_t*) (intptr_t) instance, p_mrl);
	/* media options */
	if (mediaOptions != NULL) {
		int stringCount = (*env)->GetArrayLength(env, mediaOptions);
		for (int i = 0; i < stringCount; i++) {
			jstring option = (jstring)(*env)->GetObjectArrayElement(env,
					mediaOptions, i);
	...
	for (int i = 0; i < (sizeof(mp_media_events) / sizeof(*mp_media_events));
			i++)
		libvlc_event_attach(ev_media, mp_media_events[i], vlc_event_callback,
				myVm);
...
jboolean Java_org_videolan_libvlc_LibVLC_isPlaying(JNIEnv *env, jobject thiz) {
	LOGE("Java_org_videolan_libvlc_LibVLC_isPlaying");
	libvlc_media_player_t *mp = getMediaPlayer(env, thiz);
	if (mp)
...
void Java_org_videolan_libvlc_LibVLC_play(JNIEnv *env, jobject thiz) {
	LOGE("Java_org_videolan_libvlc_LibVLC_play");
	libvlc_media_player_t *mp = getMediaPlayer(env, thiz);
	if (mp)
		libvlc_media_player_play(mp);
}
...
void Java_org_videolan_libvlc_LibVLC_pause(JNIEnv *env, jobject thiz) {
	LOGE("Java_org_videolan_libvlc_LibVLC_pause");
	libvlc_media_player_t *mp = getMediaPlayer(env, thiz);
	if (mp)
...
void Java_org_videolan_libvlc_LibVLC_stop(JNIEnv *env, jobject thiz) {
	LOGE("Java_org_videolan_libvlc_LibVLC_stop");
	libvlc_media_player_t *mp = getMediaPlayer(env, thiz);
	if (mp)
...
jint Java_org_videolan_libvlc_LibVLC_setVolume(JNIEnv *env, jobject thiz,
		jint volume) {
	libvlc_media_player_t *mp = getMediaPlayer(env, thiz);
	if (mp)
...
void Java_org_videolan_libvlc_LibVLC_setTime(JNIEnv *env, jobject thiz,
		jlong time) {
	libvlc_media_player_t *mp = getMediaPlayer(env, thiz);
	if (mp)
...
void Java_org_videolan_libvlc_LibVLC_setPosition(JNIEnv *env, jobject thiz,
		jfloat pos) {
	libvlc_media_player_t *mp = getMediaPlayer(env, thiz);
	if (mp)
...
void Java_org_videolan_libvlc_LibVLC_setTitle(JNIEnv *env, jobject thiz,
		jint title) {
	libvlc_media_player_t *mp = getMediaPlayer(env, thiz);
	if (mp)
...
jint Java_org_videolan_libvlc_LibVLC_getChapterCountForTitle(JNIEnv *env,
		jobject thiz, jint title) {
	libvlc_media_player_t *mp = getMediaPlayer(env, thiz);
	if (mp)
