...
public class AudioBrowserListAdapter extends BaseAdapter implements
		SectionIndexer {
	...
	// A list of all the sections in the list; better performance than searching
	// the whole list
	...
	class ListItem {
		...
		public ListItem(String title, String subTitle, Media media,
				boolean isSeparator) {
	 ...
	 * 
	 * @param addSeparators
	 *            True to add GUI-level separators, false to just populate the
	 *            cache
	 */
		...
		for (int i = 0; i < mItems.size(); ++i) {
			...
			if (Character.isLetter(firstChar)) {
				if (firstSeparator || firstChar != prevFirstChar) {
					if (addSeparators) {
						ListItem item = new ListItem(String.valueOf(firstChar),
								null, null, true);
						mItems.add(i, item);
	...
	/**
	 * Remove all the reference to a media in the list items. Remove also all
	 * the list items that contain only this media.
	 * 
	 * @param media
	 *            the media to remove
	 */
	public void removeMedia(Media media) {
		for (int i = 0; i < mItems.size(); ++i) {
			...
			for (int j = 0; j < item.mMediaList.size(); ++j)
				if (item.mMediaList.get(j).getLocation()
						.equals(media.getLocation())) {
	...
	public View getViewMedia(int position, View convertView, ViewGroup parent) {
		...
		/*
		 * convertView may be a recycled view but we must recreate it if it does
		 * not correspond to a media view.
		 */
		...
		if (b_createView) {
			convertView = mInflater.inflate(R.layout.audio_browser_item, null);
			holder = new ViewHolder();
			holder.title = (TextView) convertView.findViewById(R.id.song_name);
		...
		if (mItemType == ITEM_WITH_COVER) {
			...
			Bitmap cover = AudioUtil.getCover(convertView.getContext(), media,
					64);
			if (cover == null)
				cover = BitmapCache.GetFromResource(mContext, convertView,
						R.drawable.default_music_icon);
			holder.cover.setImageBitmap(cover);
			int size = (int) mContext.getResources().getDimension(
					R.dimen.audio_browser_item_size);
			paramsCover = new RelativeLayout.LayoutParams(size, size);
		} else
			paramsCover = new RelativeLayout.LayoutParams(0,
					RelativeLayout.LayoutParams.WRAP_CONTENT);
		holder.cover.setLayoutParams(paramsCover);

		holder.time.setVisibility(item.mSubTitle == null ? TextView.GONE
				: TextView.VISIBLE);
		holder.time.setText(item.mSubTitle);

		// Remove the footer if the item is just above a separator.
		holder.footer
				.setVisibility(isMediaItemAboveASeparator(position) ? View.GONE
						: View.VISIBLE);
		return convertView;
	}

	public View getViewSeparator(int position, View convertView,
			ViewGroup parent) {
		View v = convertView;
		ViewHolder holder;
		...
		/*
		 * convertView may be a recycled view but we must recreate it if it does
		 * not correspond to a separator view.
		 */
		boolean b_createView = true;
		if (v != null) {
		...
		if (b_createView) {
			...
			v = inflater.inflate(R.layout.audio_browser_separator, parent,
					false);
			holder = new ViewHolder();
	...
	public boolean isEnabled(int position) {
		return position < mItems.size()
				&& mItems.get(position).mMediaList.size() > 0;
	}
	 ...
	 * @param position
	 *            Position to retrieve in to _this_ adapter.
	 * @return The position of 'position' in the new single list, or 0 if not
	 *         found.
	 */
