...
public class AudioService extends Service {
	...
	public void onCreate() {
		...
		boolean stealRemoteControl = pref.getBoolean(
				"enable_steal_remote_control", false);
	...
	private final BroadcastReceiver serviceReceiver = new BroadcastReceiver() {
		...
		public void onReceive(Context context, Intent intent) {
			 ...
			 */
			if (action.startsWith(ACTION_REMOTE_GENERIC)
					&& !mLibVLC.isPlaying() && !hasCurrentMedia()) {
				Constant.print_i("AudioService--------serviceReceiver mLibVLC is playing");
	...
	private static class AudioServiceEventHandler extends
		...
		public void handleMessage(Message msg) {
			...
			case EventHandler.MediaPlayerPlaying:
				...
				String location = service.mLibVLC.getMediaList().getMRL(
						service.mCurrentIndex);
				long length = service.mLibVLC.getLength();
				MediaDatabase dbManager = MediaDatabase.getInstance(service
						.getApplicationContext());
				Media m = dbManager.getMedia(location);
				/**
				 * 1) There is a media to update 2) It has a length of 0
				 * (dynamic track loading - most notably the OGG container) 3)
				 * We were able to get a length even after parsing (don't want
				 * to replace a 0 with a 0)
				 */
	...
	private static class MediaListEventHandler extends
			WeakHandler<AudioService> {
		...
		public void handleMessage(Message msg) {
			...
			case EventHandler.CustomMediaListItemMoved:
				...
				// If we are in random mode, we completely reset the stored
				// previous track
	...
	private void handleVout() {
		...
		// Edited by dzt 2014.08.08
		// VideoPlayerActivity.start(VLCApplication.getAppContext(), MRL, title,
		// index, true);
	}
	...
	private boolean hasCurrentMedia() {
		return mCurrentIndex >= 0
				&& mCurrentIndex < mLibVLC.getMediaList().size();
	}
	...
	private static class AudioServiceHandler extends WeakHandler<AudioService> {
		...
		public void handleMessage(Message msg) {
			...
			case SHOW_TOAST:
				...
				Toast.makeText(service.getApplicationContext(), text, duration)
						.show();
				break;
			}
		}
	 ...
	 * 
	 * @param stopPlayback
	 *            True to also stop playback at the same time. Set to false to
	 *            preserve playback (e.g. for vout events)
	 */
	...
	private void stop() {
		...
		mLibVLC.getMediaList().getEventHandler()
				.removeHandler(mListEventHandler);
		...
		if (mNextIndex == -1) {
			...
			if (mRepeating == RepeatType.Once) {
				...
				if (mShuffling) {
					...
					} while (mNextIndex == mCurrentIndex
	...
	private void updateRemoteControlClientMetadata() {
		...
		if (mRemoteControlClient != null && media != null) {
			...
			editor.putBitmap(MetadataEditor.BITMAP_KEY_ARTWORK,
					((cover != null) ? cover.copy(cover.getConfig(), false)
							: null));
			editor.apply();
		}
	}
	...
	private final IAudioService.Stub mInterface = new IAudioService.Stub() {
		...
		public Bitmap getCoverPrev() throws RemoteException {
			if (mPrevIndex != -1)
				return AudioUtil.getCover(AudioService.this, mLibVLC
						.getMediaList().getMedia(mPrevIndex), 64);
			else
				return null;
		}
		...
		public Bitmap getCoverNext() throws RemoteException {
			if (mNextIndex != -1)
				return AudioUtil.getCover(AudioService.this, mLibVLC
						.getMediaList().getMedia(mNextIndex), 64);
			else
				return null;
		}
		 ...
		 * 
		 * @param mediaPathList
		 *            A list of locations to load
		 * @param position
		 *            The position to start playing at
		 * @param noVideo
		 *            True to disable video, false otherwise
		 * @throws RemoteException
		 */
		@Override
		public void load(List<String> mediaPathList, int position,
				boolean noVideo) throws RemoteException {
		 ...
		 * 
		 * Mainly used by VideoPlayerActivity in response to loss of video
		 * track.
		 */
		@Override
		...
		@Override
		public void loadMediaList(int position, boolean libvlcBacked,
				boolean noVideo) throws RemoteException {
			// TODO Auto-generated method stub
			Log.d("fbh", "E/ loadMediaList add start :");
			...
			if (mCurrentMedia != null) {
				...
				// updateWidget(AudioService.this);
				updateRemoteControlClientMetadata();
				Log.d("fbh", "E/ loadMediaList load read then :");
			}
			AudioService.this.saveMediaList();
			AudioService.this.saveCurrentMedia();
			...
			Log.d("fbh", "E/ load over :" + System.currentTimeMillis());
		}
	};
	...
	private void updateWidgetCover(Context context) {
		...
		Bitmap cover = hasCurrentMedia() ? AudioUtil.getCover(this,
				getCurrentMedia(), 64) : null;
	...
	private synchronized void loadLastPlaylist() {
		...
		try {
			// read CurrentMedia
			input = new FileInputStream(AudioUtil.CACHE_DIR + "/"
					+ "CurrentMedia.txt");
	...
	private synchronized void saveCurrentMedia() {
		...
		try {
			output = new FileOutputStream(AudioUtil.CACHE_DIR + "/"
					+ "CurrentMedia.txt");
	...
	// add by dzt 2014.08.08
	private void StartLoadFileThread() {
		loadMp3Thread = new HandlerThread("load_mp3_file_thread",
				Process.THREAD_PRIORITY_FOREGROUND);
		loadMp3Thread.start();
		mPlayFileHandler = new LoadFileHandler(loadMp3Thread.getLooper());
	}

	HandlerThread loadMp3Thread = null;
	Handler mPlayFileHandler = null;

	class LoadFileHandler extends Handler {
		public LoadFileHandler(Looper looper) {
			super(looper);
		}

		// Handler mPlayFileHandler = new Handler() {
		@Override
		public void handleMessage(Message msg) {
			// TODO Auto-generated method stub
			...
			if (msg.what == 3) {// next
				// mLibVLC.next();
			} else if (msg.what == 4) {// pre
				// mLibVLC.previous();
			} else if (msg.what == 1) {// 加载文件
				Bundle bundle = msg.getData();
				if (bundle != null) {
					Log.d(TAG, "fbh E/ load file thread start :");
					String filePath = bundle.getString("filePath");
					boolean noVideo = bundle.getBoolean("noVideo");
					String path = LibVLC.PathToURI(filePath);
					mLibVLC.readMedia(getApplicationContext(), path, noVideo);

					Log.d(TAG, "fbh E/ load file thread  path = " + path);
				}
			} else if (msg.what == 2) {// playindex
				Bundle bundle = msg.getData();
				if (bundle != null) {
	...
	// 快进
	private void fast_forward() {
		setUpRemoteControlClient();

		long time = mLibVLC.customizeGetTime();
		if (time >= 0) {
			mLibVLC.setTime(time + 5000);
		}
		mHandler.sendEmptyMessage(SHOW_PROGRESS);
		showNotification();
		// updateWidget(this);
